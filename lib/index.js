...
Integrator.prototype.step = function( tLimit ) {
  ...
  // Don't integrate past a limit, if provided:
  if( tLimit !== undefined ) {
  ...
  if( this.dtMaxMag !== undefined && Math.abs( this.dt ) > this.dtMaxMag ) {
    console.log('rk45(): Warning: step greater than maximum stepsize requested. dt magnitude has been limited.')
    this.dt = this.dt > 0 ? this.dtMaxMag : -this.dtMaxMag
  }

  if( this.dtMinMag !== undefined && Math.abs( this.dt ) < this.dtMinMag ) {
    console.log('rk45(): Warning: step smaller than minimum stepsize requested. dt magnitude has been limited.')
  ...
  while(true) {
    ...
    if( ! Number.isFinite(error) ) {
      throw new Error('rk45(): NaN encountered while integrating.')
    }
    ...
    if( this.dtMinMag !== undefined && Math.abs(this.dt) < this.dtMinMag ) {
      this.dt = this.dt > 0 ? this.dtMinMag : -this.dtMinMag
      console.log('rk45(): Warning: minimum stepsize reached.')
